...
<!-- 		목록 -->
  ...
  <tr>
	<td style="text-align: center !important; font-size:15px !important;"><label for="radio1-true"><input type="radio" name="radio1" id="radio1-true" value="${ResultDTO.rs_num }"></label></td>
   	<td style="text-align: center !important; font-size:15px !important;">${ResultDTO.rs_date}</td>
   	<td style="text-align: center !important; font-size:15px !important;">${ResultDTO.item_name}</td>
    <td style="text-align: center !important; font-size:15px !important;">${ResultDTO.consume}</td>
  	...
  	<fmt:formatNumber value="${ResultDTO.consume * ResultDTO.item_price}" pattern="#,###"></fmt:formatNumber>
		...
		<c:if test="${pageDTO.startPage > pageDTO.pageBlock}">
			<a href="${pageContext.request.contextPath}/store/consume?pageNum=${pageDTO.startPage - pageDTO.pageBlock}" style="font-size: 20px">Prev</a>
		</c:if>
		
		<c:if test="${pageDTO.count ne -1}">
		<c:forEach var="i" begin="${pageDTO.startPage}" end="${pageDTO.endPage}" step="1">
			<a href="${pageContext.request.contextPath}/store/consume?pageNum=${i}" style="font-size: 20px">${i}</a>
		</c:forEach>
		</c:if>
		
		<c:if test="${pageDTO.count eq -1}">
		<c:forEach var="i" begin="${resultDTO.startPage}" end="${resultDTO.endPage}" step="1">
			<a href="${pageContext.request.contextPath}/store/consumeSearch?pageNum=${i}&rs_minDate=${resultDTO.rs_minDate}&rs_maxDate=${resultDTO.rs_maxDate}" style="font-size: 20px">${i}</a>
		</c:forEach>
		</c:if>

		<c:if test="${pageDTO.endPage < pageDTO.pageCount}">
			<a href="${pageContext.request.contextPath}/store/consume?pageNum=${pageDTO.startPage + pageDTO.pageBlock}" style="font-size: 20px">Next</a>
		</c:if>
